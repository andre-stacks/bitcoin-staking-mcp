...
- Translate internal status fields into natural sentences. For timing, use the protocol-derived cycle, burn height, and approximate calendar estimate returned by the MCP; never retain a date in this skill. Mention on-chain configuration only when the user asks about readiness or configuration, or when it changes whether they can participate.
...
- State economics only from current registry evidence or explicit user scenario inputs. Never retain a current rate, duration, capacity, fee, or reward asset in this skill.
...
- For borrowing, require a current named integration plus sourced collateral, LTV, liquidation, and availability terms. Do not infer borrowing from token transferability.
