...
- Translate internal status fields into ordinary language. Use protocol-derived cycles and burn heights for eligibility, label calendar estimates approximate, and mention on-chain configuration only when the question or participation status requires it.
- Preserve route taxonomy without hard-coding current operators or capabilities. Load those claims from the current registry and explain the distinction only when it prevents confusion.
...
- State economics only when current registry evidence or explicit user inputs supply the rate and duration.
...
Never default to “wait” when a grounded preparation action exists. If no route meets every constraint, explain the conflict and the next diligence action. Keep the native-L1 direct route separate from sBTC pools, and present an LST only when the current registry attaches it to a pool. Separate protocol guarantees from application, wallet, custodian, and operational claims. Calculate gross reward when duration and rate are complete; when an applicable route or selected-LST fee is missing, label net reward unknown and never invent or default the fee to zero. Never provide a transaction-ready instruction or imply that diligence is complete.`;
