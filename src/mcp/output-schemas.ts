...
export const ProtocolStatusOutputSchema = output({
  ...
  currentCycle: z.json().optional(),
  nextCycle: z.json().optional(),
...
const ProtocolBondSchema = z.object({
  ...
  phases: z.json(), targetRateBps: z.number(), stxValueRatio: z.union([z.string(), z.number()]), minUstxRatioBps: z.number(), earlyUnlockBytes: z.string(),
...
export const YieldOutputSchema = output({
  ...
  rewardAsset: z.enum(["BTC", "sBTC", "STX", "unknown"]), pairedStxRequirement: z.json().nullable(), modelContext: z.json().nullable(),
...
export const DiligenceOutputSchema = output({
  ...
  riskSections: z.array(z.json()), operationalFit: OperationalFitSchema, missingEvidence: z.array(z.string()), nextDiligenceAction: z.string(), nextDiligenceSteps: z.array(z.string()), profile: ParticipantProfileSchema,
});
...
const OnChainReconciliationSchema = z.union([
  ...
  z.object({ status: z.enum(["found", "not_found", "conflict"]), bondIndex: z.number(), record: z.json().nullable(), network: StacksNetworkSchema, dataStatus: z.literal("live"), sources: z.array(SourceRefSchema), provenance: SourceRefSchema }).strict(),
...
export const ParticipantOutputSchema = output({ address: z.string(), network: StacksNetworkSchema, accountStatus: z.json().nullable(), stakerInfo: z.json().nullable(), bondMembership: z.json().nullable(), bondAllowanceSats: z.string().nullable(), requestedBondId: z.string().nullable(), requestedBondDataStatus: z.enum(["published", "demo"]).nullable(), networkResolution: z.object({ network: StacksNetworkSchema, selectedBy: z.enum(["bond", "request", "address", "default"]), inferredFromAddress: StacksNetworkSchema.nullable() }).strict(), componentProvenance: z.array(z.union([ProvenanceSchema, SourceRefSchema])) });
