...
2. Starts the packaged stdio server and requires the exact fifteen-tool MCP contract plus server, contract, skill, and registry versions.
...
```bash
npx -y github:andre-stacks/bitcoin-staking-mcp#v0.5.1 setup --hosts codex
npx -y github:andre-stacks/bitcoin-staking-mcp#v0.5.1 setup --hosts claude
```
...
```bash
npx -y github:andre-stacks/bitcoin-staking-mcp#v0.5.1 update
```

For harness automation:

```bash
npx -y github:andre-stacks/bitcoin-staking-mcp#v0.5.1 check --json
```

A successful result requires:

- Node 22 or newer;
- the exact fifteen-tool MCP contract and expected versions;
...
```bash
npx -y github:andre-stacks/bitcoin-staking-mcp#v0.5.1 uninstall
```
...
```bash
npx -y github:andre-stacks/bitcoin-staking-mcp#v0.5.1 uninstall --keep-skill
```
